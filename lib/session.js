...
var sess = {
  ...
  getStats: function() {
    let obj = this.getBrowserStateObject();
    let stats = [];
    for (let win of obj.windows) {
      for (let t of win.tabs) {
          let c = t.entries[t.entries.length-1];
          stats.push({
            id: c.ID,
            url: c.url,
            title: c.title,
            navCount: t.entries.length,
            birth: t.lastAccessed,
            love: t.lastAccessed - obj.session.startTime // bogus
          });
      }
   }
   return stats;
